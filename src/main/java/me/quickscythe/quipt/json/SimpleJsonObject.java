...
public class SimpleJsonObject {
     ...
     */
    public SimpleJsonObject(SimpleJsonObject jo, String... names) {
        this(names.length);
     ...
     * @throws SimpleJsonException If there is a syntax error in the source string or a
     *                       duplicated key.
     */
    public SimpleJsonObject(SimpleJsonTokener x) throws SimpleJsonException {
        this();
        char c;
        String key;
     ...
     *          the JSONObject.
     * @throws SimpleJsonException        If a value in the map is non-finite number.
     ...
     * <p>
     * The {@link SimpleJsonPropertyName} annotation can be used on a bean getter to
     ...
     * <p>
     * Similarly, the {@link SimpleJsonPropertyName} annotation can be used on non-
     ...
     * The resulting JSON object would contain <code>"FullName": "Larry Fine"</code>
     * <p>
     * The {@link SimpleJsonPropertyIgnore} annotation can be used to force the bean property
     * to not be serialized into JSON. If both {@link SimpleJsonPropertyIgnore} and
     * {@link SimpleJsonPropertyName} are defined on the same method, a depth comparison is
     * performed and the one closest to the concrete class being serialized is used.
     * If both annotations are at the same level, then the {@link SimpleJsonPropertyIgnore}
     ...
     *               from the object.
     */
    public SimpleJsonObject(Object object, String... names) {
        this(names.length);
     ...
     *                       duplicated key.
     */
    public SimpleJsonObject(String source) throws SimpleJsonException {
     ...
     * @throws SimpleJsonException If any JSONExceptions are detected.
     */
    public SimpleJsonObject(String baseName, Locale locale) throws SimpleJsonException {
        this();
...
// the deepest nested JSONObject.
                ...
                SimpleJsonObject target = this;
                for (int i = 0; i < last; i += 1) {
                    String segment = path[i];
                    SimpleJsonObject nextTarget = target.optJSONObject(segment);
                    if (nextTarget == null) {
                        nextTarget = new SimpleJsonObject();
     ...
     */
    protected SimpleJsonObject(int initialCapacity) {
...
// Shave off trailing zeros and decimal point, if possible.
     ...
     */
    public static String[] getNames(SimpleJsonObject jo) {
        if (jo.isEmpty()) {
            return null;
        }
     ...
     * @return A String.
     * @throws SimpleJsonException If n is a non-finite number.
     */
     ...
     *                               caller should catch this and wrap it in a {@link SimpleJsonException} if applicable.
     */
     ...
     * @throws SimpleJsonException If o is a non-finite number.
     */
     ...
     * brace)</small>.
     * @throws SimpleJsonException If the value is or contains an invalid number.
     */
    public static String valueToString(Object value) throws SimpleJsonException {
        // moves the implementation to JSONWriter as:
        // 1. It makes more sense to be part of the writer class
        // 2. For Android support this method is not available. By implementing it in the Writer
        //    Android users can use the writer with the built in Android JSONObject implementation.
        return SimpleJsonWriter.valueToString(value);
    }
     ...
     * @return JSONException that can be thrown.
     */
    private static SimpleJsonException wrongValueFormatException(String key, String valueType, Object value, Throwable cause) {
        return new SimpleJsonException("JSONObject[" + quote(key) + "] is not a " + valueType + " (" + value + ").", cause);
    }
     ...
     * @return this.
     * @throws SimpleJsonException        If the value is non-finite number.
     * @throws NullPointerException If the key is <code>null</code>.
     */
    public SimpleJsonObject accumulate(String key, Object value) throws SimpleJsonException {
        testValidity(value);
        Object object = this.opt(key);
        if (object == null) {
            this.put(key, value instanceof SimpleJsonArray ? new SimpleJsonArray().put(value) : value);
        } else if (object instanceof SimpleJsonArray) {
            ((SimpleJsonArray) object).put(value);
        } else {
            this.put(key, new SimpleJsonArray().put(object).put(value));
        }
        return this;
    }
     ...
     * @param value An object to be accumulated under the key.
     * @return this.
     * @throws SimpleJsonException        If the value is non-finite number or if the current value associated with
     *                              the key is not a JSONArray.
     * @throws NullPointerException If the key is <code>null</code>.
     */
    public SimpleJsonObject append(String key, Object value) throws SimpleJsonException {
        testValidity(value);
        Object object = this.opt(key);
        if (object == null) {
            this.put(key, new SimpleJsonArray().put(value));
        } else if (object instanceof SimpleJsonArray) {
            this.put(key, ((SimpleJsonArray) object).put(value));
        } else {
     ...
     * @throws SimpleJsonException if the key is not found.
     */
     ...
     * @throws SimpleJsonException if the key is not found or if the value cannot be converted
     *                       to an enum.
     */
    public <E extends Enum<E>> E getEnum(Class<E> clazz, String key) throws SimpleJsonException {
     ...
     * @param key A key string.
     * @return The truth.
     * @throws SimpleJsonException if the value is not a Boolean or the String "true" or
     *                       "false".
     */
    public boolean getBoolean(String key) throws SimpleJsonException {
     ...
     * @throws SimpleJsonException if the key is not found or if the value cannot
     *                       be converted to BigInteger.
     */
    public BigInteger getBigInteger(String key) throws SimpleJsonException {
        Object object = this.get(key);
     ...
     * @param key A key string.
     * @return The numeric value.
     * @throws SimpleJsonException if the key is not found or if the value
     *                       cannot be converted to BigDecimal.
     */
    public BigDecimal getBigDecimal(String key) throws SimpleJsonException {
        Object object = this.get(key);
     ...
     * @param key A key string.
     * @return The numeric value.
     * @throws SimpleJsonException if the key is not found or if the value is not a Number
     *                       object and cannot be converted to a number.
     */
    public double getDouble(String key) throws SimpleJsonException {
     ...
     *                       object and cannot be converted to a number.
     */
    public float getFloat(String key) throws SimpleJsonException {
        final Object object = this.get(key);
        if (object instanceof Number) {
     ...
     *                       object and cannot be converted to a number.
     */
    public Number getNumber(String key) throws SimpleJsonException {
        Object object = this.get(key);
        try {
            if (object instanceof Number) {
     ...
     *                       to an integer.
     */
    public int getInt(String key) throws SimpleJsonException {
        final Object object = this.get(key);
        if (object instanceof Number) {
     ...
     * @throws SimpleJsonException if the key is not found or if the value is not a JSONArray.
     */
    public SimpleJsonArray getJSONArray(String key) throws SimpleJsonException {
        Object object = this.get(key);
     ...
     * @throws SimpleJsonException if the key is not found or if the value is not a JSONObject.
     */
     ...
     *                       to a long.
     */
    public long getLong(String key) throws SimpleJsonException {
        final Object object = this.get(key);
        if (object instanceof Number) {
     ...
     * @throws SimpleJsonException if there is no string value for the key.
     */
     ...
     * @param key A key string.
     * @return this.
     * @throws SimpleJsonException If there is already a property with this name that is not an
     *                       Integer, Long, Double, or Float.
     */
    public SimpleJsonObject increment(String key) throws SimpleJsonException {
        ...
        if (value == null) {
            ...
            throw new SimpleJsonException("Unable to increment [" + quote(key) + "].");
        }
        return this;
    }
    ...
    public boolean isNull(String key) {
        return SimpleJsonObject.NULL.equals(this.opt(key));
    }
     ...
     * is empty.
     */
    public SimpleJsonArray names() {
        if (this.map.isEmpty()) {
            return null;
        }
        return new SimpleJsonArray(this.map.keySet());
    }
     ...
     * @return A JSONObject which is the value.
     */
    public SimpleJsonObject optJSONObject(String key) {
        Object object = this.opt(key);
        return object instanceof SimpleJsonObject ? (SimpleJsonObject) object : null;
    }
     ...
     *
     * @param bean the bean
     * @see SimpleJsonObject#SimpleJsonObject(Object)
     */
     ...
     * @return this.
     * @throws SimpleJsonException if the key is a duplicate
     */
    public SimpleJsonObject putOnce(String key, Object value) throws SimpleJsonException {
        if (key != null && value != null) {
            if (this.opt(key) != null) {
                throw new SimpleJsonException("Duplicate key \"" + key + "\"");
            }
     ...
     *              String, or the JSONObject.NULL object.
     * @return this.
     * @throws SimpleJsonException If the value is a non-finite number.
     */
    public SimpleJsonObject putOpt(String key, Object value) throws SimpleJsonException {
        if (key != null && value != null) {
            return this.put(key, value);
        }
        return this;
    ...
    public boolean similar(Object other) {
        try {
            if (!(other instanceof SimpleJsonObject)) {
                return false;
            }
            if (!this.keySet().equals(((SimpleJsonObject) other).keySet())) {
                return false;
            }
            for (final Entry<String, ?> entry : this.entrySet()) {
                String name = entry.getKey();
                Object valueThis = entry.getValue();
                Object valueOther = ((SimpleJsonObject) other).get(name);
     ...
     * @throws SimpleJsonException If any of the values are non-finite numbers.
     */
    public SimpleJsonArray toJSONArray(SimpleJsonArray names) throws SimpleJsonException {
        if (names == null || names.isEmpty()) {
            return null;
        }
        SimpleJsonArray ja = new SimpleJsonArray();
        for (int i = 0; i < names.length(); i += 1) {
     ...
     *
     * <p>If <pre>{@code indentFactor > 0}</pre> and the {@link SimpleJsonObject}
     ...
     * brace)</small> and ending with <code>}</code>&nbsp;<small>(right
     * brace)</small>.
     * @throws SimpleJsonException If the object contains an invalid number.
     */
     ...
     * @return The writer.
     * @throws SimpleJsonException if a called function has an error
     */
     ...
     * @return The writer.
     * @throws SimpleJsonException if a called function has an error or a write error
     *                       occurs
     */
    ...
    public Map<String, Object> toMap() {
        ...
        for (Entry<String, Object> entry : this.entrySet()) {
            ...
            if (entry.getValue() == null || NULL.equals(entry.getValue())) {
                value = null;
            } else if (entry.getValue() instanceof SimpleJsonObject) {
                value = ((SimpleJsonObject) entry.getValue()).toMap();
            } else if (entry.getValue() instanceof SimpleJsonArray) {
                value = ((SimpleJsonArray) entry.getValue()).toList();
            } else {
